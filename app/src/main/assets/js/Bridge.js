...
function setPageBookMark() {
   ...
   addClass(body, "page_bookmark")
}

function removePageBookMark() {
   const body = document.getElementsByTagName("body")[0];
   removeClass(body, "page_bookmark")
}
...
function setBrightness(light) {
    document.documentElement.setAttribute("style", "filter: brightness("+light+"%);");
}
...
function findSentenceWithIDInView(els) {
    ...
    for (indx in els) {
        ...
        if (document.body.scrollTop == 0) {
            ...
            if (elLeft == document.body.scrollRight) {
                currentIndex = indx;
                return element;
            }
...
$(function () {
    window.ssReader = Class({
        ...
        setFontAlef: function () {
            this.setFont('alef');
        },

        setFontBellefair: function () {
            this.setFont('bellefair');
        },

        setFontFrankRuhlLibre: function () {
            this.setFont('frankRuhlLibre');
        },

        setFontFredoka: function () {
            this.setFont('fredoka');
        },

        setFontHeebo: function () {
            this.setFont('heebo');
        },

        setFontMiriamLibre: function () {
            this.setFont('miriamLibre');
        },

        setFontNotoSansHebrew: function () {
            this.setFont('notoSansHebrew');
        },

        setFontNotoSerifHebrew: function () {
            this.setFont('notoSerifHebrew');
        },
...
function scrollToFirst() {
    ...
    switch (direction) {
        case Direction.VERTICAL:
            ...
            break;
        case Direction.HORIZONTAL:
            scrollingElement.scrollRight = 0;
...
//    console.log("-> pageCount = " + pageCount);
    ...
    EpubFragment.setHorizontalPageCount(pageCount);
}
...
function scrollToNodeOrRange(nodeOrRange) {
    ...
    switch (direction) {
        ...
        case Direction.VERTICAL:
            var topDistraction = 24//FolioWebView.getTopDistraction(DisplayUnit.DP);
            ...
            var pageBottom = scrollingElement.scrollTop + document.documentElement.clientHeight
                - 56//FolioWebView.getBottomDistraction(DisplayUnit.DP);
        ...
        case Direction.HORIZONTAL:
            var totalContentWidth = scrollingElement.scrollWidth;
            var pageWidth = scrollingElement.clientWidth;
            var totalPages = Math.ceil(totalContentWidth / pageWidth);
            var pageNumber = Math.floor(nodeOffsetLeft / pageWidth) + 1;
            pageNumber = totalPages - pageNumber;
            var scrollDistance = (pageNumber - 1) * pageWidth;
...
function onClickHighlight(element) {
    ...
    FolioWebView.setSelectionRect(rectJson.left, rectJson.top, rectJson.right, rectJson.bottom, thisHighlight.id);
}
...
function computeLastReadCfi() {
    ...
    viewportRect = null;
    EpubFragment.storeLastReadCfi(cfi);
}
